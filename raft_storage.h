...
public:
    ...
    void cover_log(int prev_idx, const std::vector<log_entry<command>> &log);
...
raft_storage<command>::raft_storage(const std::string& dir): 
    ...
    file_dir(dir), log_offset({-1}), log_end(sizeof(int)) {
    // Your code here
    ...
    log_file.read((char *)(&log_cnt), sizeof(int));
    // printf("%d logs found:\n", log_cnt);
...
raft_storage<command>::~raft_storage() {
   // Your code here
   std::unique_lock<std::mutex> lock(mtx);
   system((std::string("rm -rf ") + file_dir).c_str());
}
...
template<typename command>
void raft_storage<command>::cover_log(int prev_idx, const std::vector<log_entry<command>> &log) {
    std::unique_lock<std::mutex> lock(mtx);
    ...
    int new_size = prev_idx + log.size();
    log_file.write((char *)(&new_size), sizeof(int));

    int start_offset = prev_idx + 1 < static_cast<int>(log_offset.size()) 
        ? log_offset[prev_idx + 1] : log_end;
    log_file.seekp(start_offset, std::ios_base::beg);
    log_offset.erase(log_offset.cbegin() + prev_idx + 1, log_offset.cend());
...
void raft_storage<command>::append_log(const std::vector<log_entry<command>> &log) {
    ...
    int new_size = log_offset.size() - 1 + log.size();
    log_file.write((char *)(&new_size), sizeof(int));
